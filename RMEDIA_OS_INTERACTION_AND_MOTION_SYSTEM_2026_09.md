...
> **Update 2026-09-20 — M1 shipped.** Tokens, reduced motion, `ActionButton`, `useAction`, `usePendingGate`, `StatusTransition`, `UpdateFlash` and `aria-current` are now in production code (details and lab deviations: `RMEDIA_OS_INTERACTION_MIGRATION_MAP_2026_09.md` → "M1 as built"). Everything else below is still lab-only. Note: the scalar is `--os-motion-scale` in production (lab: `--k`), and §4's flash fade requires the transition on the element (`.os-flash`), not on `[data-flash]`.
