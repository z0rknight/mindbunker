...
> **Update 2026-09-20 — M1 shipped.** Tokens, reduced motion, `ActionButton`, `useAction`, `usePendingGate`, `StatusTransition`, `UpdateFlash` and `aria-current` are now in production code (details and lab deviations: `RMEDIA_OS_INTERACTION_MIGRATION_MAP_2026_09.md` → "M1 as built"). **M2 (2026-09-20) also shipped the client approval (§6), delivery-available and batch-progress patterns** in the client portal (see the migration map, "M2 as built"). **M3 (2026-09-20) shipped the operator patterns**: table row/cell update (§11), exception/queue resolution (§9, with the revalidation deviation noted in the migration map), live-state binding (§10), new-item arrival and queue-count value change. **M4 (2026-09-20) shipped the chart language for real evidence (§12–13, §22-23 of the lab)**: neutral bars, composition rails, source/derived/unknown encodings, exact labels. Payment (§7), Finance rows beyond the attribution panel, War Room ambient-loop retirement, sparklines/timelines are still lab-only. Note: the scalar is `--os-motion-scale` in production (lab: `--k`), and §4's flash fade requires the transition on the element (`.os-flash`), not on `[data-flash]`.
