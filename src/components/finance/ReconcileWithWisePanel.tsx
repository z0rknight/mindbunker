...
// and this component never mutates anything but cash_balance_snapshots.
...
export function ReconcileWithWisePanel({ scope }: { scope: "BUSINESS" | "PERSONAL" }) {
  ...
  const [openCurrency, setOpenCurrency] = useState<"USD" | "BRL" | null>(null);
  ...
  const load = () => {
    getReconciliation(scope).then(setRows);
  };
  ...
  const openForm = (currency: "USD" | "BRL") => {
    setOpenCurrency(currency);
    setAmount("");
  ...
  const submit = () => {
    ...
    startTransition(async () => {
      const result = await recordCashBalanceSnapshot({
        scope,
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-4 sm:p-5">
      ...
      <p className="mt-1 text-xs text-zinc-600">
        Observed balances are evidence for finding a missing movement -- recording one never
        changes revenue, expenses, FX rate, or the ledger itself.
      </p>

      <div className="mt-3 space-y-3">
        {rows.map((row) => (
          <div key={row.currency} className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-black text-white">{row.currency}</span>
              {openCurrency !== row.currency && (
                <button
                  type="button"
                  onClick={() => openForm(row.currency)}
            ...
            <div className="mt-2 grid grid-cols-3 gap-2 text-xs">
              <div>
                <p className="text-zinc-600">Ledger</p>
